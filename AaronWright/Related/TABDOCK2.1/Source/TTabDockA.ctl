...
On Error GoTo 1 'Err_UserControl_Terminate
    ...
    For x = 1 To m_DockedForms.Count
      ...
      SaveSetting App.Title, "Docking", m_DockedForms(x).Key & "Visible", m_DockedForms(x).StoreVis
    Next
...
On Error GoTo 1 'Err_UserControl_Resize
...
On Error GoTo Err_AddForm
    ...
    Set AddForm = m_DockedForms.Add(Item, Panels(Align), Style, State, Key, NewHWND)
    ...
    LockWindowUpdate Extender.Parent.hwnd
        ...
        For i = 1 To m_DockedForms.Count
            If (m_DockedForms(i).Style And tdShowInvisible) = False Then
                ...
                End If
                If m_DockedForms(i).StoreVis = False Then m_DockedForms(i).Visible = False
            End If
        Next
